...
class Tx_Barcodes_Services_UPC extends tslib_gifBuilder {
	...
	public function make() {
		...
		$upcPatterns = array(
			...
			2 => array('L' => '0010011', 'R' => '1101100'),
			3 => array('L' => '0111101', 'R' => '1000010'),
			4 => array('L' => '0100011', 'R' => '1011100'),
			5 => array('L' => '0110001', 'R' => '1001110'),
			6 => array('L' => '0101111', 'R' => '1010000'),
			7 => array('L' => '0111011', 'R' => '1000100'),
			8 => array('L' => '0110111', 'R' => '1001000'),
			9 => array('L' => '0001011', 'R' => '1110100'),
		);
		...
		// show prefix
		$this->drawPattern('00000000', $this->setup['prefix'], floor(.8 * $this->h));
		// start pattern
		$this->drawPattern($boundaryPatterns['S'], '', $this->h);
		for ($i = 0; $i < 6; $i++) {
			$char = $digits{$i};
			$this->drawPattern($upcPatterns[$char]['L'], $char, floor(.8 * $this->h));
		}
		$this->drawPattern($boundaryPatterns['M'], '', $this->h);
		for ($i = 6; $i < 12; $i++) {
			$char = $digits{$i};
			$this->drawPattern($upcPatterns[$char]['R'], $char, floor(.8 * $this->h));
		}
...
if (defined('TYPO3_MODE') && $TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/barcodes/Classes/Services/UPC.php']) {
	include_once($TYPO3_CONF_VARS[TYPO3_MODE]['XCLASS']['ext/barcodes/Classes/Services/UPC.php']);
}
